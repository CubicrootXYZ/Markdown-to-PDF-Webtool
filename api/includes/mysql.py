...
class Database:
    ...
    def connect(self):
        try:
            self.con = pymysql.connect(user=self.user, password=self.password, db=self.database, host=self.host, charset='utf8mb4', cursorclass=pymysql.cursors.DictCursor, use_unicode=True)
    ...
    def createTable(self, name, colums):
        ...
        try:
            ...
            print('Could not create Table %s in %s at %s. Got error: %s', name, self.database, self.host, e)
            return False
